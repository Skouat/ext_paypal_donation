...
class currency_controller extends admin_main
{
	...
	private function add_edit_currency_data($entity, $data): void
	{
		...
		// Override the symbol with the ICU one when the locale feature is active.
	...
	public function move(): void
	{
		...
		// move_down swaps with the next order, move_up with the previous one.
	...
	public function enable(): void
	{
		...
		if (!check_link_hash($this->request->variable('hash', ''), 'ppde_currency_state'))
		{
			trigger_error($this->language->lang('PPDE_DC_INVALID_HASH') . adm_back_link($this->u_action), E_USER_WARNING);
		}
	...
	protected function currency_assign_template_vars($data): void
	{
		...
		$this->template->assign_block_vars('currency', [
			...
			'U_ENABLE_DISABLE' => $this->u_action . '&amp;action=' . $enable_value . '&amp;' . $this->id_prefix_name . '_id=' . $data['currency_id'] . '&amp;hash=' . generate_link_hash('ppde_currency_state'),
